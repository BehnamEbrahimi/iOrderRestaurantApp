...
class MainFormViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        addMainBtn.layer.cornerRadius = 8
    ...
    @IBAction func addMainBtnPressed(_ sender: UIButton) {
        let newMain = Dish(context: self.context)
        
        newMain.name = self.nameField.text!
        newMain.type = "main"
        newMain.unitPrice = (self.priceField.text! as NSString).floatValue
        newMain.desc = self.descField.text
        
        if dishImage.image != nil {
            let dishImageData:NSData = dishImage.image!.pngData()! as NSData
        
            newMain.image = dishImageData as Data
        }
